...
from isaacgymenvs.tasks.hand_arm.base.stretch import Stretch
...
from .multi_object import ObjectAsset



class StretchMultiObject(Stretch):
    _env_cfg_path: str = 'task/StretchMultiObject.yaml'
    ...
    def register_observables(self) -> None:
        ...
        self.register_observable(
            LowDimObservable(
                name="stretch_fingertip_to_target_object_pos",
                size=3 * 2,
                get_state=lambda: self.stretch_fingertip_to_target_object_pos,
                required=True,  # NOTE: Required to compute rewards.
                requires=["stretch_fingertip_pos", "target_object_pos"],
                callback=ObservableCallback(
                    post_init=lambda: setattr(self, "stretch_fingertip_to_target_object_pos", self.target_object_pos.unsqueeze(1).repeat(1, 2, 1) - self.stretch_fingertip_pos),
                    post_step=lambda: self.stretch_fingertip_to_target_object_pos.copy_(self.target_object_pos.unsqueeze(1).repeat(1, 2, 1) - self.stretch_fingertip_pos),
                )
            )
        )
        self.register_observable(
    ...
    def _create_envs(self) -> None:
        ...
        self._acquire_robot_asset("../assets/hand_arm", "stretch/stretch.urdf")

        self.stretch_handles = []
        self.stretch_actor_indices = []
        ...
        if self.cfg_env.table_height > 0.0:
            ...
            if self.cfg_env.bin.asset == 'no_bin':
                table_asset = self.gym.create_box(self.sim, 0.75, 0.75, self.cfg_env.table_height, table_options)
        ...
        for env_index in range(self.num_envs):
            ...
            max_rigid_bodies = self.stretch_rigid_body_count + objects_rigid_body_count
            max_rigid_shapes = self.stretch_rigid_shape_count + objects_rigid_shape_count
            ...
            self.stretch_handles.append(robot_handle)
            self.stretch_actor_indices.append(actor_count)
            actor_count += 1

            # Create table actor.
            if self.cfg_env.table_height > 0.0:
                if self.cfg_env.bin.asset == 'no_bin':
                    table_pose = gymapi.Transform(p=gymapi.Vec3(0.2925, 0.51, self.cfg_env.table_height/2))
                else:
        ...
        self.stretch_actor_indices = torch.tensor(self.stretch_actor_indices, dtype=torch.int32, device=self.device)
        self.stretch_rigid_body_env_indices = [self.gym.get_actor_rigid_body_index(self.env_ptrs[0], self.stretch_handles[0], i, gymapi.DOMAIN_ENV) for i in range(self.stretch_rigid_body_count)]
