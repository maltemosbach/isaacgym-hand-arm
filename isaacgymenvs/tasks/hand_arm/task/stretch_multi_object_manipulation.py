from isaacgymenvs.tasks.hand_arm.env.stretch_multi_object import StretchMultiObject
...
class StretchMultiObjectManipulation(StretchMultiObject):
    _task_cfg_path = 'task/StretchMultiObjectManipulation.yaml'
    ...
    def reset_idx(self, env_ids):
        if self.objects_dropped:
            ...
            self._reset_stretch(env_ids, reset_dof_pos=self.cfg_base.asset.joint_configurations.bringup)
        ...
        self._reset_stretch(env_ids, reset_dof_pos=self.cfg_base.asset.joint_configurations.reset)
    ...
    def _update_rew_buf(self):
        ...
        for reward_term, scale in self.cfg_task.rl.reward.items():
            if reward_term == 'lifting':
                ...
                target_object_pos_expanded = self.target_object_pos.unsqueeze(1).repeat(1, self.stretch_fingertip_pos.shape[1], 1)
                #fingertip_distance = torch.min(torch.norm(self.fingertip_pos - target_object_pos_expanded, dim=2), dim=1).values
                fingertip_distance = torch.norm(self.stretch_fingertip_pos - target_object_pos_expanded, dim=2)
                fingertip_distance = 4 * torch.sum(fingertip_distance, dim=1)  # 3 * to account for difference to SIH hand
